...
exports.getPaymentOfAnArtist = async (req, res) => {
  try {
    const servicePayments = await Payment.find({
      ...
      .populate('userId')
      .populate('serviceId');
    const albumPayments = await Payment.find({
      artistId: req.params.artistId,
      isAlbum: true,
    }).populate('userId');
...
exports.updateAnEmployee = async (req, res) => {
  try {
    const { username, phone, email, address, accountNo, ifscCode, upiId } =
      req.body;
        ...
        for (let i = 0; i < artists.length; i++) {
          for (let j = 0; j < allPayments.length; j++) {
            ...
            if (
              ...
              allPayments[j].artistId._id.toString().trim()
            ) {
              // console.log(j);
      ...
      if (payments.length == 0) {
        ...
        for (let i = 0; i < users.length; i++) {
          for (let j = 0; j < allPayments.length; j++) {
            // console.log(allPayments[j].artistId.toString().trim())
            if (
              users[i]._id.toString().trim() ==
              allPayments[j].userId._id.toString().trim()
            ) {
              // console.log(j);
              payments.push(allPayments[j]);
            }
  ...
  try {
    ...
    for (let i = 0; i < employees.length; i++) {
      ...
      for (let j = 0; j < artists.length; j++) {
        if (
          employees[i]._id.toString().trim() ==
          artists[j].assignedEmployee.toString().trim()
        )
          arts++;
      }
      retArr.push({
        ...
        startDate: employees[i].createdAt,
        totalArtists: arts,
        blocked: employees[i].blocked,
      });
    }
    res.status(200).send(retArr);
  } catch (error) {
  ...
  try {
    ...
    let balance =
      artistBalance - parseInt(req.body.amount) > 0
        ? artistBalance - parseInt(req.body.amount)
        : 0;
