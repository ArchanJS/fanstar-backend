  ...
  try {
    ...
    if (!service) res.status(400).json({ error: 'Service not found!' });
      ...
      if (uBalance >= servicePrice) {
        ...
        aBalance =
          aBalance + (servicePrice * parseFloat(artist.commission)) / 100.0;
        ...
        res.status(400).json({ error: "User doesn't have enough balance!" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: 'Something went wrong!' });
  ...
  try {
    ...
    if (!image) res.status(400).json({ error: 'Image not found!' });
      ...
      if (uBalance >= imagePrice) {
        ...
        let aBalance = parseFloat(artist.balance);
        aBalance =
          aBalance + (imagePrice * parseFloat(artist.commission)) / 100.0;
        aBalance = aBalance.toString();
        await Artist.updateOne(
          { _id: artist._id },
          {
  ...
  try {
    ...
    if (!album) res.status(400).json({ error: 'No album exists!' });
      ...
      if (uBalance >= albumPrice) {
        ...
        let aBalance = parseFloat(artist.balance);
        aBalance =
          aBalance + (albumPrice * parseFloat(artist.commission)) / 100.0;
        aBalance = aBalance.toString();
        await Artist.updateOne(
          { _id: album.postedBy },
          {
...
exports.unsubscribe = async (req, res) => {
  try {
    ...
    await Payment.updateMany(
      { artistId: req.body.artistId, userId: req.user._id, isAlbum: true },
      {
        $set: {
          status: 'completed',
        },
...
exports.checkIfSubscribed = async (req, res) => {
  try {
    ...
    if (isSubscriber == false)
      res.status(200).json({ isSubscriber: false, recentlyExpired: false });
    else {
...
exports.giveFeedback = async (req, res) => {
  try {
    ...
    await Service.findOneAndUpdate(
      { _id: payment.serviceId, 'feedbacks.userId': { $ne: req.user._id } },
      {
        $addToSet: {
          feedbacks: {
            userId: req.user._id,
  ...
  try {
    const service = await Service.findOne({
      $and: [
        ...
        { serviceName: { $regex: req.params.serviceName, $options: 'i' } },
      ],
    });
...
exports.completePayment = async (req, res) => {
  try {
    ...
    if (
      payment &&
      payment.userId.toString().trim() == req.user._id.toString().trim()
    ) {
...
exports.dedudctBalanceWhileChatting = async (req, res) => {
  try {
    ...
    if (uBalance >= chatPrice) {
      aBalance = aBalance + (chatPrice * parseFloat(artist.commission)) / 100.0;
      aBalance = aBalance.toString();
      await Artist.updateOne(
        { _id: artist._id },
        {
...
exports.giveEmoji = async (req, res) => {
  try {
    ...
    if (uBalance >= price) {
      ...
      let aBalance = parseFloat(artist.balance);
      aBalance = aBalance + (price * parseFloat(artist.commission)) / 100.0;
      aBalance = aBalance.toString();
      await Artist.updateOne(
        { _id: artistId },
        {
